...
class SearchPage(tk.Frame):
    ...
    def _on_detail_mousewheel(self, event):
        """Прокрутка правой панели мышкой/тачпадом с любого вложенного элемента."""
        try:
            if hasattr(event, "delta") and event.delta:
    ...
    def _create_result_item(self, item):
        ...
        # Фиксированная сетка строки:
        # 0 — синяя иконка тикера, 1 — текст, 2 — кнопки, 3 — цена.
    ...
    def _draw_candlestick_chart(self, parent, candles_df, ticker):
        """
        Рисует реальный свечной график за последний месяц.
        Данные берутся из MOEX через fetch_full_candles().
        """
        ...
        # Обертка нужна, чтобы сам график стоял по центру правой панели,
        # а не прилипал к левому краю при растягивании окна.
        ...
        try:
            ...
            # Берём последние 30 торговых свечей
    ...
    def _format_company_description(self, description):
        ...
        # Если в JSON уже есть переносы строк — сохраняем их.
        if "\n" in description:
            return description

        # Разбиваем длинную строку из JSON на читаемые абзацы.
        parts = []
    ...
    def _build_detail_panel(self, secid, name, price_info, hist, details, candles_30d=None):
        ...
        # Реальный свечной график за месяц
        ...
        # Колесо мыши внутри текста прокручивает всю правую панель, а не только Text.
        ...
        # После построения карточки привязываем прокрутку мышкой ко всем элементам правой панели.
